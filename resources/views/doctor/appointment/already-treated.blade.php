...
<style>
    ...
    <div class="row">
       ...
       <a href="{{ route('doctor.treatment.edit',$appointment->id) }}" class="btn btn-primary ml-3 edit-btn">{{ app()->currentLocale() == 'ar' ? $websiteLang->where('lang_key','edit')->first()->custom_lang : $websiteLang->where('lang_key','edit')->first()->default_lang }}</a>
       @endif
        </div>
    </div>
