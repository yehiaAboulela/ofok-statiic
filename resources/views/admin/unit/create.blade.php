...
@section('title')
<title>{{ app()->currentLocale() == 'ar' ? $websiteLang->where('lang_key','unit')->first()->custom_lang : $websiteLang->where('lang_key','unit')->first()->default_lang }}</title>
@endsection
        ...
        <div class="col-md-10">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">{{ app()->currentLocale() == 'ar' ? $websiteLang->where('lang_key','unit_form')->first()->custom_lang : $websiteLang->where('lang_key','dep_form')->first()->default_lang }}</h6>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.unit.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="name_ar">{{ __('dashboard.unit.Name In Arabic') }}</label>
                        ...
                        <div class="form-group">
                            <label for="description_ar">{{ __('dashboard.unit.Description In Arabic') }}</label>
                            <textarea class="summernote" id="description_ar" name="description_ar"></textarea>
                        </div>
