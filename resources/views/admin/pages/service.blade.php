        ...
        <div class="col-md-8">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">{{ $websiteLang->where('lang_key','service')->first()->custom_lang }}</h6>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.service.page.update') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="show_navbar" id="show_navbar" class="form-control">
                                <option {{ $navigation->show_service==1 ? 'selected':'' }} value="1">{{ $websiteLang->where('lang_key','yes')->first()->custom_lang }}</option>
                                <option {{ $navigation->show_service==0 ? 'selected':'' }} value="0">{{ $websiteLang->where('lang_key','no')->first()->custom_lang }}</option>
                            </select>
                        </div>
